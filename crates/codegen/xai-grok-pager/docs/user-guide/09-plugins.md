...
| `--plugin-dir` (CLI flag, `your-own-ai-build agent`) | Process -- loaded for that agent process only | Trusted automatically |
| `.grok/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.your-own-ai-build/plugins/` | User -- personal plugins for every project | Trusted automatically |
...
```bash
your-own-ai-build plugin list [--json] [--available]   # List installed plugins (--available requires --json)
your-own-ai-build plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
your-own-ai-build plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
your-own-ai-build plugin update [<name>]               # Omit the name to update all plugins
your-own-ai-build plugin enable <name>
your-own-ai-build plugin disable <name>
your-own-ai-build plugin details <name>                # Show the plugin's component inventory
your-own-ai-build plugin validate [<path>]             # Validate plugin.json (default: current directory)
your-own-ai-build plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `your-own-ai-build plugin install <source>` without `--trust` and Grok prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
```bash
your-own-ai-build plugin marketplace list [--json]
your-own-ai-build plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
your-own-ai-build plugin marketplace remove <url>      # Git URL or local path of a configured source
your-own-ai-build plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
your-own-ai-build plugin marketplace add my-org/team-plugins
your-own-ai-build plugin marketplace list
your-own-ai-build plugin install my-org/team-plugins --trust
your-own-ai-build plugin list
your-own-ai-build plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `your-own-ai-build plugin` CLI.
...
Configure plugin directories and per-plugin state in `~/.your-own-ai-build/config.toml`:
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `your-own-ai-build plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.

### Hide the plugins UI

To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.your-own-ai-build/pager.toml`:
...
Place this file at `~/.your-own-ai-build/settings.json` or `~/.claude/settings.json`.
...
Grok trusts plugins from `~/.your-own-ai-build/plugins/` automatically. Project plugins in `.grok/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
your-own-ai-build plugin install <source> --trust
```
...
Run `your-own-ai-build inspect` to see every discovered plugin and what it provides:

```bash
your-own-ai-build inspect          # Show plugins with their skills, agents, hooks, and MCP servers
your-own-ai-build inspect --json   # Emit machine-readable JSON
```
