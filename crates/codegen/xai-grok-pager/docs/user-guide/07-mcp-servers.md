...
MCP servers are configured in `~/.your-own-ai-build/config.toml` under `[mcp_servers.<name>]` sections.
...
> - `config.toml` — user-level (`~/.your-own-ai-build/config.toml`) **or repo-level**
...
your-own-ai-build mcp list
your-own-ai-build mcp list --json          # Machine-readable output
...
your-own-ai-build mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
your-own-ai-build mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
your-own-ai-build mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
your-own-ai-build mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
your-own-ai-build mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
your-own-ai-build mcp remove github

# Diagnose a server's configuration and connectivity
your-own-ai-build mcp doctor               # Check every configured server
your-own-ai-build mcp doctor github        # Check one server
your-own-ai-build mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `your-own-ai-build mcp add` writes to `~/.your-own-ai-build/config.toml` (`--scope user`). Use `--scope project` to write to `.grok/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `your-own-ai-build mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`your-own-ai-build mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.your-own-ai-build/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native Grok config | `~/.your-own-ai-build/config.toml`, `.grok/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.your-own-ai-build/config.toml` or the corresponding environment variable (`GROK_CURSOR_MCPS_ENABLED`, `GROK_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `your-own-ai-build inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).
...
You must authenticate OAuth-based MCP servers before you can use them. Grok stores the resulting tokens under `~/.your-own-ai-build/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
For stdio servers, Grok captures the process's standard error to `~/.your-own-ai-build/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.your-own-ai-build/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `your-own-ai-build inspect` to see all loaded MCP servers and their sources:

```bash
your-own-ai-build inspect          # Human-readable
your-own-ai-build inspect --json   # Machine-readable
```
