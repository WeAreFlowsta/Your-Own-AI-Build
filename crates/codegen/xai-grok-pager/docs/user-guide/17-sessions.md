...
Sessions are identified by a unique session ID (a UUIDv7 when Grok generates it; a client may supply its own ID with `-s`) and stored on disk under `~/.your-own-ai-build/sessions/`. Set `GROK_HOME` to override the base directory; when it is unset, Grok uses `~/.your-own-ai-build`.
...
Run `your-own-ai-build --resume` without an ID to resume the most recent session for the current directory.

### From the Welcome Screen

When you launch `your-own-ai-build`, the welcome screen lists recent sessions for the current directory. Select one to resume it.
...
your-own-ai-build -p "Continue where we left off" -r <session-id>
...
List or search sessions from the command line. `your-own-ai-build sessions` requires a subcommand:

```bash
# List recent sessions for the current directory
your-own-ai-build sessions list

# Limit the number of results (default 20)
your-own-ai-build sessions list --limit 50

# Search sessions by keyword (matches titles and prompts)
your-own-ai-build sessions search "rate limit"
```

`your-own-ai-build sessions list` shows sessions for the current working directory, grouped by worktree label. Each row lists the session ID, the creation and update dates, the source status, and the summary. `your-own-ai-build sessions search` combines a local SQLite index with remote results.
...
Resume a session in a fresh worktree with `your-own-ai-build -w -r <session-id>`.
...
The smaller state files -- `summary.json`, `plan.json`, and `signals.json` -- are plain JSON rather than JSONL. JSONL is the source of truth for session content; `your-own-ai-build sessions search` additionally maintains a local SQLite FTS5 index over session titles and prompts for fast keyword search.
