...
- **`your-own-ai-build dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.your-own-ai-build/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Your Own AI Build · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Your Own AI Build's sibling views (
`tasks_pane`):
...
rebound via `~/.your-own-ai-build/config.toml`.
...
process. The plan's Phase 4 ("supervisor / `your-own-ai-build --bg`") would list
