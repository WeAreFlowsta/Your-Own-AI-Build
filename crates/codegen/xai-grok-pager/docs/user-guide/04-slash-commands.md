...
`--fullscreen` CLI flags are likewise session-scoped. To make plain `your-own-ai-build` open
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter appears as a slash command. (A skill turned off via `/skills` is not advertised.) For example, if you have a skill at `~/.your-own-ai-build/skills/commit/SKILL.md`, you can invoke it with:
