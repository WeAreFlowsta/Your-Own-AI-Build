...
|----------|-------|-------------|
| `~/.your-own-ai-build/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.your-own-ai-build/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.your-own-ai-build/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |
...
Ask Grok to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.your-own-ai-build/memory/MEMORY.md` for cross-project preferences:
...
Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.your-own-ai-build/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.
...
You can edit memory files directly under `~/.your-own-ai-build/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
```
Memory saved to ~/.your-own-ai-build/memory/MEMORY.md
```
...
The `your-own-ai-build memory` command manages memory from the shell. It has one subcommand, `clear`:

```bash
# Clear workspace memory (MEMORY.md, sessions/, and index.sqlite). This is the default scope.
your-own-ai-build memory clear

# The same scope, stated explicitly
your-own-ai-build memory clear --workspace

# Clear the global MEMORY.md
your-own-ai-build memory clear --global

# Clear both workspace and global memory
your-own-ai-build memory clear --all

# Skip the confirmation prompt (-y is the short form)
your-own-ai-build memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.your-own-ai-build/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.your-own-ai-build/memory/` for external edits and reindex |
...
By default, Grok watches `~/.your-own-ai-build/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `your-own-ai-build inspect` output.
2. Check the flag: `your-own-ai-build --experimental-memory` or `GROK_MEMORY=1`.
...
```bash
ls ~/.your-own-ai-build/memory/
cat ~/.your-own-ai-build/memory/MEMORY.md
$EDITOR ~/.your-own-ai-build/memory/MEMORY.md
```
