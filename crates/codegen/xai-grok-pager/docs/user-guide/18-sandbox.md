...
```bash
# Run with workspace sandbox (read everywhere, write to CWD + temp dirs + ~/.your-own-ai-build/)
your-own-ai-build --sandbox workspace

# Read-only mode (read everywhere, write only to ~/.your-own-ai-build/ + temp dirs)
your-own-ai-build --sandbox read-only

# Most restrictive profile (read CWD + system paths, write CWD + temp dirs + ~/.your-own-ai-build/, no child network)
...
| `workspace`           | Everywhere         | CWD + `~/.your-own-ai-build/` + `/tmp` + `/var/tmp`         | Allowed       | Normal development                |
| `devbox`              | Everywhere         | All top-level dirs except `/data`              | Allowed       | Disposable dev VMs                |
| `read-only`           | Everywhere         | `~/.your-own-ai-build/` + `/tmp` + `/var/tmp`               | Blocked¹      | Exploration, code review          |
| `strict`              | CWD + system paths | CWD + `~/.your-own-ai-build/` + `/tmp` + `/var/tmp`         | Blocked¹      | Untrusted code                    |
...
**workspace** -- The recommended profile for everyday development. The agent can read any file on the system (for understanding dependencies, system libraries, etc.) but can only write to the current working directory, `~/.your-own-ai-build/`, and temp directories (`/tmp`, `/var/tmp`, plus the macOS temp dirs). Network access is allowed for tools like `web_search` and MCP servers.
...
**read-only** -- Use when you want the agent to analyze code without modifying your project files. The agent can read everything but can only write to `~/.your-own-ai-build/` (needed for session persistence) and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

**strict** -- The most restrictive profile, for reviewing untrusted code. The agent can only read files within the current working directory and essential system paths. Writes are limited to CWD, `~/.your-own-ai-build/`, and temp directories. Child-process network access is blocked on Linux (no-op on macOS).
...
Create custom sandbox profiles in `~/.your-own-ai-build/sandbox.toml` (global) or `.grok/sandbox.toml` (per-project):
...
```bash
your-own-ai-build --sandbox project
```
...
for the life of the session**. When you resume it (`your-own-ai-build --resume <id>`,
`your-own-ai-build --continue`, or `your-own-ai-build -r`), Grok restores that same profile automatically —
...
Sandbox events are logged to `~/.your-own-ai-build/sandbox-events.jsonl` for debugging. Events include:
