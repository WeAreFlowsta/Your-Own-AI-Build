...
3. **config.toml** (`~/.your-own-ai-build/config.toml`)
...
Location: `~/.your-own-ai-build/config.toml`
...
`[ui] vim_mode` in `~/.your-own-ai-build/config.toml` immediately and applies it to every
...
`your-own-ai-build` launches. Configure it from `/settings` → **Default screen mode**
...
MCP servers can also be configured per-project in `.grok/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.your-own-ai-build/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.grok/config.toml` (current dir) > `<repo-root>/.grok/config.toml` > `~/.your-own-ai-build/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).
...
`your-own-ai-build inspect` reports cells that still need session-start resolution as
...
`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.your-own-ai-build/config.toml`.
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `your-own-ai-build`. |
...
Location: `~/.your-own-ai-build/pager.toml`
...
| Path | Description |
|------|-------------|
| `~/.your-own-ai-build/config.toml` | Main configuration file |
| `~/.your-own-ai-build/pager.toml` | TUI appearance configuration |
...
| `~/.your-own-ai-build/lsp.json` | LSP server configuration (user-scoped) |
| `~/.your-own-ai-build/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
...
|------|--------------------|
| `.grok/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.your-own-ai-build/config.toml`) |
...
|--------|----------|-------|
| User | `~/.your-own-ai-build/lsp.json` | All projects |
...
2. **User** -- `~/.your-own-ai-build/lsp.json`
