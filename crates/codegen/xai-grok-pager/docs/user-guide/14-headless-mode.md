...
By default, each `your-own-ai-build -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
your-own-ai-build -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
your-own-ai-build -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
your-own-ai-build -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
your-own-ai-build -p "What's the secret number?" --resume abc123
```
...
your-own-ai-build -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
your-own-ai-build --prompt-file ./prompt.txt
```
...
your-own-ai-build -p "Format all files" --yolo

# Run tests and fix failures
your-own-ai-build -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
- Resume: `your-own-ai-build -p "continue" --resume "<id>"` or `your-own-ai-build -p "continue" --continue`
