...
Your Own AI Build supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.your-own-ai-build/hooks/`
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.your-own-ai-build/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `your-own-ai-build plugin marketplace add <source>`)
...
Sources land in `~/.your-own-ai-build/config.toml`:
...
Or in `~/.your-own-ai-build/settings.json` / `~/.claude/settings.json`:
