...
Sample hooks for Grok. Copy to `~/.your-own-ai-build/hooks/` to enable globally, or to `<project>/.grok/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.your-own-ai-build/hooks/bin
cp examples/hooks/safe-shell.json ~/.your-own-ai-build/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.your-own-ai-build/hooks/bin/
chmod +x ~/.your-own-ai-build/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.your-own-ai-build/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.your-own-ai-build/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.your-own-ai-build/hooks/bin/
chmod +x ~/.your-own-ai-build/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.your-own-ai-build/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.your-own-ai-build/hooks/bin
cp examples/hooks/session-log.json ~/.your-own-ai-build/hooks/
cp examples/hooks/bin/session-log.sh ~/.your-own-ai-build/hooks/bin/
chmod +x ~/.your-own-ai-build/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.your-own-ai-build/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.your-own-ai-build/hooks/bin
cp examples/hooks/tool-logger.json ~/.your-own-ai-build/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.your-own-ai-build/hooks/bin/
chmod +x ~/.your-own-ai-build/hooks/bin/tool-logger.sh
```
...
Remove the JSON file from `~/.your-own-ai-build/hooks/`. The hook stops running on the next session.