...
name: create-skill
description: >
  Interactively create a new Your Own AI Build skill (SKILL.md + optional scripts/references).
  Use when the user wants to create a skill, scaffold a skill, or runs /create-skill.
metadata:
  short-description: "Create a new Your Own AI Build skill"
---

# Create Skill

Interactively gather requirements from the user and create a fully working Your Own AI Build skill on disk.
...
2. **Scope** - present the user with two options:
   ...
   - **User**: `~/.your-own-ai-build/skills/<name>/SKILL.md` - available in all projects
...
- Trigger phrases and keywords so Your Own AI Build knows when to auto-invoke it
...
Where `<SKILL_DIR>` is:
- User scope: `~/.your-own-ai-build/skills/<name>`
...
2. Tell the user the skill is ready and how to use it:
   ...
   - Automatic: Your Own AI Build will invoke it when the description matches user intent
