                ...
                maybe_cmd = cmd_rx.recv() => {
                    ...
                    match cmd {
                        ...
                        SessionCommand::SetAutoMode { enabled } => {
                            ...
                            // The LLM side-query wires only when the config
                            // opts into the model classifier; the default is
                            // the built-in rule-based judge alone.
                            if enabled && crate::util::config::auto_mode_uses_model_classifier() {
