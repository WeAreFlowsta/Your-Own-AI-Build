...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("grok-build", "Your Own AI Build"),
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("grok-build", "Your Own AI Build")];
        let current = acp::ModelId::from("grok-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            ...
            category: "model".to_string(),
            label: "Your Own AI Build".to_string(),
            description: None,
            selected: true,
        };
        ...
        assert_eq!(v.get("label").and_then(|x| x.as_str()), Some("Your Own AI Build"));
    ...
    fn acp_config_options_map_model_and_effort_selectors() {
        ...
        let expected = vec![
            acp::SessionConfigOption::select(
                ...
                "grok-4.5",
                vec![
                    acp::SessionConfigSelectOption::new("grok-build", "Your Own AI Build"),
