...
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "your-own-ai-build: --plugin-dir is ignored in leader mode; run with --no-leader to \
...
async fn run_agent_command(
    ...
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("your-own-ai-build: {warning}");
    }
...
mod tests {
    ...
    fn version_output_writer_preserves_channel_aware_contract() {
        ...
        for (label, expected_suffix) in [
            ...
            assert!(output.starts_with("your-own-ai-build "));
