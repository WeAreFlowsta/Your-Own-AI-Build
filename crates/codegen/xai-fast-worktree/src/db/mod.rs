...
pub fn resolve_grok_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("YOUR_OWN_AI_BUILD_HOME") {
        return Ok(PathBuf::from(v));
    }
    if let Ok(v) = std::env::var("GROK_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home = PathBuf::from(std::env::var("HOME").context("neither $YOUR_OWN_AI_BUILD_HOME nor $HOME is set")?);
    ...
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".your-own-ai-build"))
}
