...
/// layout is no longer scanned — it pulled vendor default skills into Your Own AI Build sessions.
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Your Own AI Build.
