            ...
            match cmd {
                ...
                PermissionCommand::Request {
                    ...
                    let prompt_outcome = tokio::select! {
                        outcome = prompter.request_with_reason(&access, &tool_call_update, protected_edit, hook_ask.as_ref(), Some(prompt_trigger)) => outcome,
...
mod tests {
    ...
    /// Auto mode accepts ordinary file edits via the fast path when they are
    /// INSIDE the session cwd (this fork bounds auto to the project folder;
    /// outside-cwd edits are covered by
    /// `auto_mode_edit_outside_workspace_prompts`).
    #[tokio::test]
    ...
    async fn auto_mode_edit_outside_workspace_prompts() {
        ...
        local
            .run_until(async {
                ...
                let d = tokio::time::timeout(
                    std::time::Duration::from_secs(5),
                    decide(&mgr, AccessKind::Edit(outside), mk("tc-edit-out")),
                )
                .await
                ...
                let d = tokio::time::timeout(
                    std::time::Duration::from_secs(5),
                    decide(&mgr, AccessKind::Edit("/etc/hosts".into()), mk("tc-edit-protected")),
                )
                .await
                ...
                let d = tokio::time::timeout(
                    std::time::Duration::from_secs(5),
                    decide(&mgr, AccessKind::Edit("../escape.rs".into()), mk("tc-edit-dotdot")),
                )
                .await
