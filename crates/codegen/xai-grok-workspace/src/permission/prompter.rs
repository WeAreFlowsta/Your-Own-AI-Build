...
impl AcpPrompter {
    ...
    pub async fn request(
        ...
        self.request_with_reason(access, tool_call_update, protected_edit, hook_ask, None)
            .await
    }
    ...
    pub async fn request_with_reason(
        ...
        let outcome = match &self.hub_permission {
            ...
            None => {
                ...
                // Upstream's meta (bash scope selection / protected-edit
                // payload), with the prompt reason added on top.
                let mut meta = self.permission_request_meta(access, protected_edit, hook_ask);
