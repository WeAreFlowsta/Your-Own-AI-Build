...
fn grok_home_in(home: &Path) -> PathBuf {
    dunce::canonicalize(home)
        ...
        // Your Own AI Build keeps its config under its own name, not .grok.
...
/// Resolve the home from the environment (fresh, no cache); `None` if neither
/// resolves. `$YOUR_OWN_AI_BUILD_HOME` wins; `$GROK_HOME` is honored for
/// compatibility with pre-rebrand setups.
...
pub fn resolve_grok_home_with_source() -> Option<(PathBuf, GrokHomeSource)> {
    resolve_grok_home_from(
        std::env::var_os("YOUR_OWN_AI_BUILD_HOME")
...
mod tests {
    ...
    fn empty_env_falls_through_to_os_home() {
        ...
        assert_eq!(
            resolved,
            Some((
                dunce::canonicalize(tmp.path()).unwrap().join(".your-own-ai-build"),
    ...
    fn default_grok_home_has_no_verbatim_prefix() {
        ...
        assert!(home.ends_with(".your-own-ai-build"));
    }
